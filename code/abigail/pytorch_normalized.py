...
class EEGEpochDataset(Dataset):
    def __init__(self, csv_path):
        ...
        # Temporary storage grouped by subject
        ...
        for subject, epochs in subject_data.items():
            # Stack all time points of this subject
            all_x = np.concatenate([ep[0] for ep in epochs], axis=0)  # (N*T, 19, 5)
...
# -------------------------------------------------
# Continuous 70/30 split (first 70% train, last 30% val per subject)
# -------------------------------------------------
def subject_aware_split(dataset, train_ratio=0.7):
    ...
    for subj, indices in subject_to_indices.items():
        ...
        train_indices.extend(indices[:n_train])     # first 70%
        val_indices.extend(indices[n_train:])       # last 30%
...
# -------------------------------------------------
# Clean CNN + Pooling model (no VQ)
# -------------------------------------------------
class EEGPoolingCNN(nn.Module):
    def __init__(self, num_classes=4):
        super().__init__()

        # Input shape after permute: (B, 5, T, 19)
        self.features = nn.Sequential(
            nn.Conv2d(5, 32, kernel_size=(3, 3), padding=1),
            nn.BatchNorm2d(32),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=(1, 2)),          # pool electrodes
...
# -------------------------------------------------
def per_class_accuracy(preds, labels, num_classes=4):
    accs = []
...
def train():
    ...
    model = EEGPoolingCNN(num_classes=4).to(device)
    ...
    class_weights = torch.tensor(
        [total / max(label_counts[i], 1) for i in range(4)],
        dtype=torch.float32
    ).to(device)
    ...
    for epoch in range(1, 41):
        ...
        class_accs = per_class_accuracy(all_preds, all_labels)

        print(f"\nEpoch {epoch:02d}")
        print(f"  Train Loss: {train_loss:.4f} | Train Acc: {train_acc:.3f}")
        print(f"  Val Acc: {val_acc:.3f}")
        print(f"  Per-class Val Acc → Rest: {class_accs[0]:.3f} | Low: {class_accs[1]:.3f} | "
              f"Medium: {class_accs[2]:.3f} | High: {class_accs[3]:.3f}")

        if val_acc > best_val_acc:
            best_val_acc = val_acc
            torch.save(model.state_dict(), "eeg_cnn_baseline_best.pt")
